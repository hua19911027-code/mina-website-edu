...
- **新建 A5-每年9月30日封存舊學年題庫**（`H540ksT2ZiAPIeEK`，仿 A3/A4 風格，`errorWorkflow` 接 F2，`active: true`）：
  ...
  - **這次修復後同樣尚未實測**，建議 Manko 再測一次「Execute workflow」確認批次封存段真的能跑
...
下一步：
- **A5 再測一次**：114標題空格已修、批次封存段的 bug 也已修（改用 A2 同款 pattern），但改完還沒實測過，9/30前建議 Manko 再點一次「Execute workflow」確認真的能封存成功
